...
- Link
  ...
  - Link"는 애플리케이션 내에서 다른 경로로 이동하기 위한 HTML 앵커 태그`<a>`를 생성한다. "Link" 컴포넌트를 사용하면 사용자가 클릭할 때 페이지를 새로고침하지 않고도 라우터가 링크된 경로로 이동할 수 있다. 예를 들어, 사용자가 클릭하면 URL이 변경되고, 해당 경로에 맞는 컴포넌트가 렌더링됩니다.
...
- useNavigate
  ...
  - React Router v6에서 도입된 훅

  - useNavigate는 함수를 반환하며, 이 함수를 호출하여 프로그래밍 방식으로 경로를 변경할 수 있다.
  - useNavigate를 사용하면 컴포넌트 내에서 동적으로 경로를 조작할 수 있습니다. 일반적으로 이 훅은 이벤트 핸들러나 조건부 논리와 함께 사용됩니다.
